...
export const handler = async (event) => {
  ...
  try {
    ...
    await sendSlackMessage("ASG 인스턴스 Scale Up 완료 이벤트 수신");
    await sleep(60 * 1000);

    await sendSlackMessage("health check 시작");
    ...
    if (!isHealthy) {
      await sendSlackMessage("rollback 요청");
      await sqs.sendFailMessage();
      await sendSlackMessage("rollback 요청 완료");
      return;
    }

    const cloudFront = new CloudFront(process.env.DISTRIBUTION_ID);

    await sendSlackMessage("CF 업데이트 요청");
    const isDeployed = await cloudFront.updateOriginDomainName(await ec2.getPublicDnsName());
    await sendSlackMessage(`CF 업데이트 ${isDeployed ? "성공" : "실패"}`);

    await sqs.sendDelaySuccessMessage();
    await sendSlackMessage("SQS success 메세지 전송 완료");
  } catch (error) {
    console.error(error);
    await sendSlackMessage(`[backend_delivery_processing] 에러발생\n${error}`);
    throw error;
  }
};