...
export const handler = async (event) => {
  ...
  try {
    await sendSlackMessage(`${isSuccessMessage ? "" : "[롤백] "} ASG Scale Down 진행`);
    ...
    await sendSlackMessage("ASG Scale Down 요청");
    await autoScaling.scaleDown(isSuccessMessage);
    const isTerminated = await autoScaling.checkInstanceTerminated();
    await sendSlackMessage(`ASG 사이즈 조정 ${isTerminated ? "완료" : "실패"}`);
  } catch (error) {
    console.error(error);
    await sendSlackMessage(`[backend_delivery_complete] 에러발생\n${error}`);
    throw error;
  }
};
