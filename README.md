...
## (다른 객체의 참조가 필요한 경우) 말단 노드에서는 private readonly 필드와 헬퍼 메서드를 제공하고, 그 외 노드는 자식 노드를 public readonly 필드를 선언한다.

- 설명:
    - 루트 노드, 중간 노트의 캡슐화를 포기하는 대신 불필요한 위임 메서드를 제거한다.
...
## 말단 노드에서만 `CommonInfra`의 필드와 메서드를 사용한다.
